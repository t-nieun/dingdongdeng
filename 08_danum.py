    ...
    if now_rmse > 500:  # 피아노 소리가 들리지 않을 때는 계산하지 않음 (들어온 데이터의 크기로 분석)
        ...
        if len(std_peaks) > 0 and now_rmse > 600:
            ...
            if keep_keep_rmse < keep_rmse and keep_rmse > now_rmse and keep_keep_keeep_rmse < keep_rmse and \
                ...
                str_keep_peaks2 = str(keep_peaks2) + str(scale(keep_peaks2 * x_interval)[0])
                plt.annotate('before_peaks : %s' % str_keep_peaks, xy=(11, 10), xytext=(4000, 2800000), size=10, ha='right',
                             va='center')
                plt.annotate('after_peaks : %s' % str_keep_peaks1, xy=(11, 10), xytext=(4000, 2500000), size=10, ha='right',
                             va='center')
                plt.annotate('after2_peaks : %s' % str_keep_peaks2, xy=(11, 10), xytext=(4000, 2200000), size=10,
                             ha='right',
                             va='center')
                plt.xlim(0, 4000)
                plt.ylim(0, 10000000)
