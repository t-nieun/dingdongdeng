...
def multiple_freq_decrease(y_, peak_):
    ...
    if len(peak_) == 1:
        ...
        for i in range(len(peak_)-1):
            next_peak_ind = peak_[i + 1]
            y_[next_peak_ind-1] = y_[next_peak_ind-1] - sub_val[i]
            y_[next_peak_ind] = y_[next_peak_ind] - sub_val[i]
            y_[next_peak_ind+1] = y_[next_peak_ind+1] - sub_val[i]
    else:
        for i in range(3):
            next_peak_ind = peak_[i + 1]
...
#while (True):
    ...
    peaks, _ = find_peaks(y, height=2000)
    frequency_ = find_index(y, 5)

    val = peaks * x_interval
