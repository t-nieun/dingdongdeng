...
def x_linspace(fre_key):
    linspace_Dict = {'D3': 7,}
    return linspace_Dict[fre_key]
        ...
        if now_rmse > 2000:  # 피아노 소리가 들리지 않을 때는 계산하지 않음 (들어온 데이터의 크기로 분석)
            ...
            if len(std_peaks) > 0 and now_rmse > 4000:
                if keep_keep_rmse < keep_rmse and keep_rmse > now_rmse and keep_keep_keeep_rmse < keep_rmse and \
                    ...
                    plt.plot(after_peaks * x_interval, before_decrease_y[after_peaks], "rx")  # 찾은 계이름 빨간색X로 표시
                    plt.plot(x, before_decrease_y, 'g')  # 감소후 데이터 녹색으로 표시
                    plt.plot(x, before_origin_y, 'r--')  # 감소전 데이터 빨간색점선으로 표시

                    std_y = np.ones(int(n / 2)) * before_threshold
                    plt.plot(x, std_y,)
                    plt.legend(('', 'after_decrease', 'before_decrease'))
                    plt.annotate('threshold : %d' % (before_threshold), xy=(11, 10), xytext=(4000, 7500), size=10, ha='right',
                                 va='center')
                    plt.annotate('%s' % str(keep_gyename[0]), xy=(11, 10), xytext=(4000, 10000000), size=10, ha='right', va='center')
